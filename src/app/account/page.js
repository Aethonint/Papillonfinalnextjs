...
import { ShoppingBag, AlertTriangle, Truck, Package } from "lucide-react";
...
export default function AccountPage() {
  ...
  useEffect(() => {
    async function fetchStats() {
      ...
      try {
        const res = await fetch("http://localhost:8000/api/profile/stats", {
          headers: {
  ...
  // Define the cards using the real data
  const statCards = [
    { 
      label: "Total Orders", 
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 md:grid-cols-4 gap-8">
      ...
      <main className="col-span-3 space-y-10">
        ...
        {/* RECENT ACTIVITY / CTA (Optional) */}
        <div className="bg-blue-50 rounded-3xl p-8 border border-blue-100 flex justify-between items-center">
             <div>
